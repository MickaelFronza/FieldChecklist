...
import { ChecklistExecution, ExecutionItem, Machine, TemplateItem, User } from '../models';
...
export const listExecutions = asyncHandler(async (req: Request, res: Response) => {
  const { date, machineId, operatorId, status } = req.query as Record<string, string | undefined>;
